########################################################################################################################
# Utilities to work with the Free Music Archive (FMA) dataset
#
# Copied from: https://github.com/mdeff/fma/blob/master/utils.py
########################################################################################################################
...
class FreeMusicArchive:
    ...
    def _get_data(self, dataset, fma_id, fields=None):
        ...
        if r_id != str(fma_id):
            raise Exception("The received id {} does not correspond to" "the requested one {}".format(r_id, fma_id))
        if fields is None:
            return data
